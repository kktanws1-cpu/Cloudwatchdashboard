...
async function fetchRealAlarms() {
  try {
    const res  = await fetch("https://54.251.122.0.nip.io/alarms");
...
async function fetchPerformance() {
  try {
    const res  = await fetch("https://54.251.122.0.nip.io/performance");
    const data = await res.json();
    return data;
  } catch { return null; }
}
...
async function fetchCost() {
  try {
    const res  = await fetch("https://54.251.122.0.nip.io/cost");
    const data = await res.json();
    return data;
  } catch { return null; }
}
...
async function fetchCanary() {
  try {
    const res  = await fetch("https://54.251.122.0.nip.io/canary");
    const data = await res.json();
    return data;
  } catch { return null; }
}
...
async function fetchServices() {
  try {
    const res  = await fetch("https://54.251.122.0.nip.io/services");
    const data = await res.json();
    return data;
  } catch { return null; }
}
...
async function fetchRdsPerf() {
  try {
    const res  = await fetch("https://54.251.122.0.nip.io/rds-performance");
    const data = await res.json();
    return data;
  } catch { return null; }
}
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0,overflow:"auto"}}>
        ...
        {!loadingAlarms&&alarms.length===0&&(
          <div style={{background:C.amberBg,borderBottom:`1px solid ${C.amberBdr}`,padding:"10px 24px",fontSize:12,color:C.amber,fontWeight:600}}>
            ⚠ No alarms found. Make sure the backend is running at https://54.251.122.0.nip.io
          </div>
        )}
