...
async function fetchRealAlarms() {
  try {
    const res  = await fetch("http://47.129.171.222:3001/alarms");
...
async function fetchPerformance() {
  try {
    const res  = await fetch("http://47.129.171.222:3001/performance");
    const data = await res.json();
    return data;
  } catch { return null; }
}
...
async function fetchCost() {
  try {
    const res  = await fetch("http://47.129.171.222:3001/cost");
    const data = await res.json();
    return data;
  } catch { return null; }
}
...
async function fetchCanary() {
  try {
    const res  = await fetch("http://47.129.171.222:3001/canary");
    const data = await res.json();
    return data;
  } catch { return null; }
}
...
async function fetchServices() {
  try {
    const res  = await fetch("http://47.129.171.222:3001/services");
    const data = await res.json();
    return data;
  } catch { return null; }
}
...
async function fetchRdsPerf() {
  try {
    const res  = await fetch("http://47.129.171.222:3001/rds-performance");
    const data = await res.json();
    return data;
  } catch { return null; }
}
...
const NAV = [
  ...
  { id:"alarms",      icon:"ðŸ””", label:"Alarms & Alerts",   badge:12, badgeColor:C.red     },
  { id:"incidents",   icon:"âš ",  label:"Incidents",         badge:4,  badgeColor:C.orange  },
  { id:"services",    icon:"âŠ¡",  label:"Services"           },
  { id:"performance", icon:"ðŸ“ˆ", label:"Performance"        },
  { id:"canary",      icon:"ðŸ¦", label:"Canary Monitoring"  },
  { id:"infra",       icon:"â˜",  label:"Infrastructure"     },
  { id:"security",    icon:"ðŸ›¡", label:"Security"           },
  { id:"cost",        icon:"ðŸ’°", label:"Cost Management"    },
...
/* â”€â”€ Sparkline SVG â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
function SectionHeader({title,action,badge}) {
  return (
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
      ...
      {action&&<button onClick={action.fn} style={{background:"none",border:"none",color:C.primary,fontSize:12,cursor:"pointer",fontWeight:600,fontFamily:C.font}}>{action.label} â†’</button>}
    </div>
  );
}

/* â”€â”€ Rule-Based Analysis Engine â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
function analyseEC2(perf) {
  const s = perf.summary; const instances = perf.instances || [];
  const findings = []; const actions = []; let health = "GOOD";

  // CPU
  if (s.avgCpu > 90)  { health="CRITICAL"; findings.push({sev:"CRITICAL", metric:"CPU Utilization", val:`${s.avgCpu}%`, msg:"CPU is critically high (>90%). Instances are overloaded and may become unresponsive."}); actions.push("ðŸ”´ Immediately scale up instance type (e.g. t3.micro â†’ t3.medium) or add instances behind a load balancer.","ðŸ”´ Identify the runaway process: SSH in and run `top` or `htop` to find CPU-heavy processes.","ðŸ”´ Enable EC2 Auto Scaling with a CPU threshold of 70% to automatically add capacity."); }
  else if (s.avgCpu > 80) { health="WARNING"; findings.push({sev:"WARNING", metric:"CPU Utilization", val:`${s.avgCpu}%`, msg:"CPU is high (>80%). Performance degradation is likely under load."}); actions.push("ðŸŸ  Consider upgrading instance type or enabling Auto Scaling.","ðŸŸ  Review application for inefficient loops or blocking operations."); }
  else if (s.avgCpu < 5)  { findings.push({sev:"INFO", metric:"CPU Utilization", val:`${s.avgCpu}%`, msg:"CPU is very low (<5%). Instance may be over-provisioned."}); actions.push("ðŸ”µ Consider downsizing to a smaller instance type to reduce cost (e.g. use AWS Compute Optimizer)."); }
  else { findings.push({sev:"OK", metric:"CPU Utilization", val:`${s.avgCpu}%`, msg:"CPU is within normal range."}); }

  // Network
  if (s.avgNetworkIn > 500)  { findings.push({sev:"WARNING", metric:"Network In", val:`${s.avgNetworkIn} MB/hr`, msg:"High inbound network traffic. Check for unexpected data ingestion or DDoS activity."}); actions.push("ðŸŸ  Review VPC Flow Logs for unexpected traffic sources.","ðŸŸ  Consider enabling AWS Shield or WAF if traffic is external."); }
  if (s.avgNetworkOut > 500) { findings.push({sev:"WARNING", metric:"Network Out", val:`${s.avgNetworkOut} MB/hr`, msg:"High outbound network traffic. May incur significant data transfer costs."}); actions.push("ðŸŸ  Use CloudFront CDN to reduce origin data transfer costs.","ðŸŸ  Review application for unnecessary large payload responses."); }
  ...
  if (hotInstances.length) { findings.push({sev:"WARNING", metric:"Hot Instances", val:hotInstances.map(i=>`${i.instanceId} (${i.cpu.avg}%)`).join(", "), msg:`${hotInstances.length} instance(s) have CPU > 80%. Uneven load distribution detected.`}); actions.push("ðŸŸ  Review load balancer configuration to ensure even traffic distribution across instances."); }

  if (!actions.length) actions.push("âœ… No immediate actions required. Continue monitoring.");
...
function analyseRDS(rdsPerf) {
  ...
  clusters.forEach(c => {
    ...
    // CPU
    if (m.cpu.avg > 90)      { health="CRITICAL"; allFindings.push({sev:"CRITICAL", metric:`${label} CPU`, val:`${m.cpu.avg}%`, msg:"Database CPU critically high. Queries may timeout."}); allActions.push(`ðŸ”´ ${label} Immediately upgrade instance class (e.g. db.t3.micro â†’ db.t3.medium).`,`ðŸ”´ ${label} Run slow query log analysis: enable \`log_min_duration_statement\` in Parameter Group.`); }
    else if (m.cpu.avg > 70) { health="WARNING";  allFindings.push({sev:"WARNING",  metric:`${label} CPU`, val:`${m.cpu.avg}%`, msg:"Database CPU is high. Risk of query slowdowns under load."}); allActions.push(`ðŸŸ  ${label} Review and optimise slow queries using Performance Insights.`,`ðŸŸ  ${label} Add missing indexes on frequently queried columns.`); }
    else { allFindings.push({sev:"OK", metric:`${label} CPU`, val:`${m.cpu.avg}%`, msg:"CPU is normal."}); }

    // Free Storage
    if (m.freeStorage.avg < 2)       { health="CRITICAL"; allFindings.push({sev:"CRITICAL", metric:`${label} Free Storage`, val:`${m.freeStorage.avg} GB`, msg:"Critically low storage. Database may stop accepting writes soon."}); allActions.push(`ðŸ”´ ${label} Immediately increase allocated storage in RDS console (storage auto-scaling recommended).`); }
    else if (m.freeStorage.avg < 10) { health=health==="CRITICAL"?health:"WARNING"; allFindings.push({sev:"WARNING", metric:`${label} Free Storage`, val:`${m.freeStorage.avg} GB`, msg:"Storage is running low (<10GB)."}); allActions.push(`ðŸŸ  ${label} Enable RDS Storage Auto Scaling with a maximum threshold.`,`ðŸŸ  ${label} Purge old data or archive to S3 using AWS DMS.`); }
    ...
    if (totalIOPS > 1000)      { health=health==="CRITICAL"?health:"WARNING"; allFindings.push({sev:"WARNING", metric:`${label} IOPS`, val:`Read: ${m.readIOPS.avg}/s  Write: ${m.writeIOPS.avg}/s`, msg:`Total IOPS is high (${totalIOPS.toFixed(0)}/s). Risk of I/O bottleneck.`}); allActions.push(`ðŸŸ  ${label} Add indexes to reduce full table scans (check via slow query log).`,`ðŸŸ  ${label} Enable ElastiCache (Redis) to cache frequent read queries and reduce DB load.`,`ðŸŸ  ${label} Consider Read Replicas to offload read traffic.`,`ðŸŸ  ${label} Upgrade to gp3 storage with provisioned IOPS if on gp2.`); }
    else if (totalIOPS > 500)  { allFindings.push({sev:"INFO", metric:`${label} IOPS`, val:`Read: ${m.readIOPS.avg}/s  Write: ${m.writeIOPS.avg}/s`, msg:"IOPS is moderately elevated. Monitor for increases."}); allActions.push(`ðŸ”µ ${label} Review query execution plans with EXPLAIN ANALYZE.`); }
    ...
    // Latency
    if (m.readLatency.avg > 20 || m.writeLatency.avg > 20) { health=health==="CRITICAL"?health:"WARNING"; allFindings.push({sev:"WARNING", metric:`${label} Latency`, val:`Read: ${m.readLatency.avg}ms  Write: ${m.writeLatency.avg}ms`, msg:"Query latency is high (>20ms). Users may experience slow responses."}); allActions.push(`ðŸŸ  ${label} Enable RDS Performance Insights to identify the top SQL statements by load.`,`ðŸŸ  ${label} Check for lock contention and long-running transactions.`); }
    ...
    // Memory
    if (m.memory.avg < 0.2)      { health=health==="CRITICAL"?health:"WARNING"; allFindings.push({sev:"WARNING", metric:`${label} Freeable Memory`, val:`${m.memory.avg} GB`, msg:"Very low freeable memory. Risk of swap usage and performance degradation."}); allActions.push(`ðŸŸ  ${label} Upgrade instance class for more RAM.`,`ðŸŸ  ${label} Tune \`shared_buffers\` and \`work_mem\` in Parameter Group.`); }
    else { allFindings.push({sev:"OK", metric:`${label} Freeable Memory`, val:`${m.memory.avg} GB`, msg:"Memory is adequate."}); }

    // Connections
    if (m.connections.avg > 100) { allFindings.push({sev:"INFO", metric:`${label} DB Connections`, val:`${m.connections.avg}`, msg:"Connection count is elevated. Consider connection pooling."}); allActions.push(`ðŸ”µ ${label} Implement RDS Proxy to pool and manage database connections efficiently.`); }
    else { allFindings.push({sev:"OK", metric:`${label} DB Connections`, val:`${m.connections.avg}`, msg:"Connection count is normal."}); }
  });

  if (!allActions.length) allActions.push("âœ… All RDS metrics are within healthy thresholds. No actions required.");
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(15,20,40,0.45)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000}}>
      <div style={{background:C.surface,borderRadius:C.r,width:"min(780px,96vw)",maxHeight:"88vh",display:"flex",flexDirection:"column",boxShadow:C.shadowLg,border:`1px solid ${C.border}`}}>
        ...
        <div style={{padding:"18px 24px",borderBottom:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <div style={{fontSize:10,color:C.primary,fontFamily:C.mono,fontWeight:700,letterSpacing:"0.1em",marginBottom:3}}>âš¡ RULE-BASED Â· ANALYSIS ENGINE</div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <button onClick={onClose} style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:C.rSm,color:C.textSub,padding:"7px 14px",cursor:"pointer",fontSize:12}}>âœ• Close</button>
          </div>
        </div>
...
/* â”€â”€ AI Modal â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
function AIModal({alarms,target,perfTarget,onClose}) {
  ...
  const run=useCallback(async()=>{
    ...
    } catch { setText("âš  Could not reach analysis API. Check your API key."); }
    setLoading(false);
  ...
  const md=t=>t
    ...
    .replace(/^- (.+)$/gm,`<div style="display:flex;gap:8px;margin:4px 0;color:${C.textSub}"><span style="color:${C.primary}">â–¸</span><span>$1</span></div>`)
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(15,20,40,0.45)",backdropFilter:"blur(6px)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000}}>
      <div style={{background:C.surface,borderRadius:C.r,width:"min(760px,95vw)",maxHeight:"86vh",display:"flex",flexDirection:"column",boxShadow:C.shadowLg,border:`1px solid ${C.border}`}}>
        <div style={{padding:"18px 24px",borderBottom:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div>
            <div style={{fontSize:10,color:C.primary,fontFamily:C.mono,fontWeight:700,letterSpacing:"0.1em",marginBottom:3}}>âš¡ CLAUDE AI Â· ANALYSIS ENGINE</div>
          ...
          <div style={{display:"flex",gap:8}}>
            <button onClick={run} style={{background:C.primaryLight,border:`1px solid ${C.blueBdr}`,borderRadius:C.rSm,color:C.primary,padding:"7px 14px",cursor:"pointer",fontSize:12,fontFamily:C.mono,fontWeight:600}}>â†º Re-run</button>
            <button onClick={onClose} style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:C.rSm,color:C.textSub,padding:"7px 14px",cursor:"pointer",fontSize:12}}>âœ• Close</button>
          </div>
        </div>
        <div style={{padding:"22px 24px",overflowY:"auto",flex:1,fontSize:13,color:C.textSub,lineHeight:1.8,fontFamily:C.font}}>
          {loading
            ?<div style={{textAlign:"center",padding:"48px 0"}}>
               <div style={{fontSize:32,marginBottom:14,animation:"spin 1.4s linear infinite",display:"inline-block"}}>âš¡</div>
               <div style={{color:C.primary,fontFamily:C.mono,fontSize:13,fontWeight:600}}>Analysing alarm patternsâ€¦</div>
             </div>
...
/* â”€â”€ Alarm Detail Modal â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
      ...
      <div style={{background:C.surface,borderRadius:C.r,width:"min(680px,95vw)",maxHeight:"88vh",overflowY:"auto",boxShadow:C.shadowLg,border:`1px solid ${C.border}`}}>
        <div style={{padding:"20px 24px",borderBottom:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div>
            ...
            <div style={{fontSize:12,color:C.textMute,fontFamily:C.mono,marginTop:2}}>{alarm.namespace} Â· {alarm.resource}</div>
          </div>
          <div style={{display:"flex",gap:8}}>
            <button onClick={onAnalyze} style={{background:C.primary,border:"none",borderRadius:C.rSm,color:"#fff",padding:"8px 16px",cursor:"pointer",fontSize:12,fontFamily:C.mono,fontWeight:700}}>âš¡ AI Analyze</button>
            <button onClick={onClose} style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:C.rSm,color:C.textSub,padding:"8px 14px",cursor:"pointer"}}>âœ•</button>
          </div>
        </div>
...
/* â”€â”€ Performance Metric Card â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
function PerfCard({label, value, unit, delta, deltaUp, sparkData, color, loading}) {
  return (
    <div style={{background:C.bg,borderRadius:C.rSm,padding:"12px 14px",border:`1px solid ${C.border}`}}>
      ...
      {loading
        ? <div style={{fontSize:13,color:C.textMute,fontFamily:C.mono,marginBottom:6}}>Loadingâ€¦</div>
...
/* â”€â”€ EC2 Instance Table â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€ */
...
export default function App() {
  ...
  /* Performance display values â€” real if available, else show dashes */
    ...
    <div style={{display:"flex",minHeight:"100vh",background:C.bg,fontFamily:C.font,color:C.text}}>
      ...
      <div style={{width:200,background:C.surface,borderRight:`1px solid ${C.border}`,display:"flex",flexDirection:"column",flexShrink:0,position:"sticky",top:0,height:"100vh",overflowY:"auto"}}>
        <div style={{padding:"20px 18px 16px",borderBottom:`1px solid ${C.border}`}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <div style={{width:32,height:32,background:"linear-gradient(135deg,#4f6ef7,#7c3aed)",borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontSize:15,fontWeight:800}}>â˜</div>
        ...
        <div style={{margin:"0 8px 8px",background:"linear-gradient(135deg,#eef1ff,#f5f3ff)",borderRadius:C.r,padding:"14px 12px",border:`1px solid ${C.blueBdr}`}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
            <div style={{width:28,height:28,background:"linear-gradient(135deg,#4f6ef7,#7c3aed)",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>ðŸ¤–</div>
          ...
          <button onClick={()=>{setAiTarget(null);setShowAI(true);}} style={{width:"100%",background:C.primary,border:"none",borderRadius:C.rXs,color:"#fff",padding:"7px",fontSize:11,fontFamily:C.mono,fontWeight:700,cursor:"pointer"}}>âœ¦ Ask AI</button>
        </div>
      </div>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0,overflow:"auto"}}>
        ...
        <div style={{background:C.surface,borderBottom:`1px solid ${C.border}`,padding:"0 24px",display:"flex",alignItems:"center",gap:16,height:60,flexShrink:0,position:"sticky",top:0,zIndex:10}}>
          <div>
            ...
            <div style={{fontSize:11,color:C.textMute}}>Real-time AWS CloudWatch Â· {lastRefresh?`Last refresh: ${lastRefresh.toLocaleTimeString("en-SG")}`:"Loadingâ€¦"}</div>
          </div>
          <div style={{flex:1}}/>
          <div style={{position:"relative"}}>
            <span style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",color:C.textMute,fontSize:13}}>ðŸ”</span>
            <input placeholder="Search anythingâ€¦" style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:C.rSm,padding:"7px 12px 7px 32px",fontSize:12,color:C.text,width:200,fontFamily:C.font}}/>
          </div>
          <div style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:C.rSm,padding:"7px 12px",fontSize:12,color:C.textSub,display:"flex",alignItems:"center",gap:6}}>
            ðŸ“… {new Date().toLocaleDateString("en-SG",{month:"short",day:"numeric",year:"numeric"})}
          </div>
          <button onClick={loadAll} style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:C.rSm,padding:"7px 10px",cursor:"pointer",fontSize:14,color:C.textSub}}>
            {(loadingAlarms||loadingPerf)?<span style={{animation:"spin 1s linear infinite",display:"inline-block"}}>â†»</span>:"â†»"}
          </button>
          <div style={{position:"relative"}}>
            <button style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:C.rSm,padding:"7px 10px",cursor:"pointer",fontSize:14,color:C.textSub}}>ðŸ””</button>
        ...
        {(loadingAlarms||loadingPerf)&&(
          <div style={{background:C.primaryLight,borderBottom:`1px solid ${C.blueBdr}`,padding:"8px 24px",fontSize:12,color:C.primary,display:"flex",alignItems:"center",gap:8}}>
            ...
            Fetching real-time data from AWS CloudWatchâ€¦
          </div>
        )}
        {!loadingAlarms&&alarms.length===0&&(
          <div style={{background:C.amberBg,borderBottom:`1px solid ${C.amberBdr}`,padding:"10px 24px",fontSize:12,color:C.amber,fontWeight:600}}>
            âš  No alarms found. Make sure the backend is running at http://47.129.171.222:3001
          </div>
        )}

        <div style={{padding:"22px 24px",flex:1}}>

          {/* â”€â”€ Non-overview placeholder views â”€â”€ */}
          {activeNav==="alarms" && (
            <div>
                ...
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                  ...
                  <tbody>{canaryData.canaries.map((c,i)=>(
                    <tr key={i} style={{borderBottom:`1px solid ${C.borderSoft}`}}>
                      ...
                      <td style={{padding:"10px 10px",color:C.textMute,fontFamily:C.mono,fontSize:11}}>{c.lastRun ? new Date(c.lastRun.startedAt).toLocaleString("en-SG") : "â€”"}</td>
                      ...
                      <td style={{padding:"10px 10px"}}>
                        {c.successRate !== null
                          ...
                          : <span style={{color:C.textMute,fontSize:11}}>â€”</span>}
                      </td>
                    </tr>
                  ))}</tbody>
                </table>
          ...
          {activeNav==="performance" && (
            <div>
              ...
              <div style={{background:C.surface,borderRadius:C.r,padding:"18px 20px",boxShadow:C.shadow,border:`1px solid ${C.border}`,marginBottom:28}}>
                ...
                {loadingPerf ? <div style={{textAlign:"center",padding:"20px",color:C.textMute,fontSize:12}}>Loading EC2 metricsâ€¦</div> : <EC2Table instances={perf?.instances}/>}
              </div>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
                ...
                {rdsPerf?.clusters?.length > 0 && <button onClick={()=>{setAiPerfTarget({type:"rds",clusters:rdsPerf.clusters});setAiTarget(null);setShowAI(true);}}
                  ...
                  âš¡ Analyse RDS with AI
                </button>}
              </div>
              {!rdsPerf
                ? <div style={{textAlign:"center",padding:"30px",color:C.textMute,fontSize:12}}>Loading RDS metricsâ€¦</div>
          ...
          {activeNav==="cost" && (
            <div>
              <div style={{fontSize:20,fontWeight:800,color:C.text,marginBottom:18}}>Cost Management {costData?.month ? `â€” ${costData.month}` : ""}</div>
          ...
          {activeNav==="overview" && <div>{/* â”€â”€ Row 1: KPI Cards â”€â”€ */}
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:22}}>
            <div style={{background:C.surface,borderRadius:C.r,padding:"18px 20px",boxShadow:C.shadow,border:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:14}}>
              <div style={{width:44,height:44,borderRadius:10,background:alarmCount===0?C.greenBg:C.redBg,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}>{alarmCount===0?"ðŸ’š":"âš ï¸"}</div>
              <div style={{flex:1}}>
            ...
            <div style={{background:C.surface,borderRadius:C.r,padding:"18px 20px",boxShadow:C.shadow,border:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:14}}>
              <div style={{width:44,height:44,borderRadius:10,background:"#fff8e7",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}>ðŸ””</div>
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:11,marginTop:2}}><span style={{color:C.red,fontWeight:700}}>{alarmCount} In Alarm</span><span style={{color:C.textMute}}> Â· </span><span style={{color:C.green,fontWeight:700}}>{okCount} OK</span></div>
              </div>
            </div>
            {/* EC2 CPU Card â€” real data */}
            <div style={{background:C.surface,borderRadius:C.r,padding:"18px 20px",boxShadow:C.shadow,border:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:14}}>
              <div style={{width:44,height:44,borderRadius:10,background:cpu>80?C.redBg:C.blueBg,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}>ðŸ’»</div>
              <div style={{flex:1}}>
                <div style={{fontSize:11,color:C.primary,fontWeight:700,marginBottom:2}}>EC2 Avg CPU</div>
                <div style={{fontSize:28,fontWeight:800,color:cpu>80?C.red:C.text,fontFamily:C.mono}}>{loadingPerf?"â€¦":cpu!=null?`${cpu}%`:"N/A"}</div>
                <div style={{fontSize:11,color:C.textMute,marginTop:2}}>{perf?.instanceCount??0} instance{perf?.instanceCount!==1?"s":""} Â· Live</div>
              </div>
            </div>
            <div style={{background:C.surface,borderRadius:C.r,padding:"18px 20px",boxShadow:C.shadow,border:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:14}}>
              <div style={{width:44,height:44,borderRadius:10,background:C.blueBg,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22}}>ðŸŒ</div>
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:11,color:C.textMute,marginTop:2}}>Singapore Â· Live</div>
              </div>
            </div>
          </div>

          {/* â”€â”€ Row 2: Overview + Severity + Top Alarms â”€â”€ */}
            ...
            <div style={{background:C.surface,borderRadius:C.r,padding:"18px 20px",boxShadow:C.shadow,border:`1px solid ${C.border}`}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:20}}>
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{marginTop:8,fontSize:10,color:C.textMute}}>ðŸ”„ Auto-refresh every 60s</div>
                </div>
              </div>
            </div>
            <div style={{background:C.surface,borderRadius:C.r,padding:"18px 20px",boxShadow:C.shadow,border:`1px solid ${C.border}`}}>
            ...
            <div style={{background:C.surface,borderRadius:C.r,padding:"18px 20px",boxShadow:C.shadow,border:`1px solid ${C.border}`}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {alarms.filter(a=>a.state==="ALARM").slice(0,4).length>0
                  ?alarms.filter(a=>a.state==="ALARM").slice(0,4).map(a=>(
                    <div key={a.name} onClick={()=>setSelected(a)} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 10px",borderRadius:C.rSm,background:C.bg,cursor:"pointer",border:`1px solid ${C.border}`,transition:"all 0.15s"}}
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        <div style={{fontSize:10,color:C.textMute,fontFamily:C.mono}}>{a.resource} Â· {a.type}</div>
                      </div>
                  ...
                  :<div style={{textAlign:"center",padding:"20px 0",color:C.textMute,fontSize:12}}>âœ… No alarms currently in ALARM state</div>
                }
              </div>
            </div>
          </div>

          {/* â”€â”€ Row 3: All Alarms Table â”€â”€ */}
          ...
          {/* â”€â”€ Row 4: Performance Overview â€” REAL EC2 DATA â”€â”€ */}
          <div style={{background:C.surface,borderRadius:C.r,padding:"18px 20px",boxShadow:C.shadow,border:`1px solid ${C.border}`,marginBottom:22}}>
            <SectionHeader
            ...
            <div style={{background:C.bg,borderRadius:C.rSm,padding:"14px 16px",border:`1px solid ${C.border}`}}>
              ...
              {loadingPerf
                ?<div style={{textAlign:"center",padding:"20px",color:C.textMute,fontSize:12}}>Loading EC2 metrics from CloudWatchâ€¦</div>
                :<EC2Table instances={perf?.instances}/>
              }
            </div>
          </div>

          {/* â”€â”€ Row 5: Canary + Cost + Service Status â”€â”€ */}
          <div style={{display:"grid",gridTemplateColumns:"1fr 1.2fr 1fr",gap:14,marginBottom:22}}>
            <div style={{background:C.surface,borderRadius:C.r,padding:"18px 20px",boxShadow:C.shadow,border:`1px solid ${C.border}`}}>
              <SectionHeader title="Canary Monitoring"/>
              {!canaryData
                ? <div style={{textAlign:"center",padding:"20px",color:C.textMute,fontSize:12}}>Loading canary dataâ€¦</div>
                : <>
            ...
            <div style={{background:C.surface,borderRadius:C.r,padding:"18px 20px",boxShadow:C.shadow,border:`1px solid ${C.border}`}}>
              ...
              <div style={{marginBottom:8}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  <span style={{fontSize:26,fontWeight:800,color:C.text,fontFamily:C.mono}}>
                    {costTotal !== null ? `$${costTotal.toLocaleString()}` : "Loadingâ€¦"}
                  </span>
                </div>
              </div>
              ...
              <div style={{display:"flex",alignItems:"center",gap:12,marginTop:12}}>
                <Donut segments={costDonut} size={80} thickness={14}
                  label={costTotal !== null ? `$${costTotal >= 1000 ? (costTotal/1000).toFixed(1)+"K" : costTotal}` : "â€¦"}
                  sublabel="Total"/>
                <div style={{flex:1}}>
                  {costDonut.map((r,i)=>{
          ...
          {/* â”€â”€ Row 6: Quick Actions â”€â”€ */}
          <div style={{background:C.surface,borderRadius:C.r,padding:"18px 20px",boxShadow:C.shadow,border:`1px solid ${C.border}`}}>
            <SectionHeader title="Quick Actions"/>
            <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:10}}>
              {[
                {icon:"âš ï¸",label:"Create Incident",   bg:"#fff5f5",     color:C.red},
                {icon:"ðŸ¦",label:"Run Canary Test",   bg:C.blueBg,      color:C.blue},
                {icon:"ðŸ“‹",label:"View Logs",          bg:C.purpleBg,    color:C.purple},
                {icon:"ðŸ’°",label:"Cost Report",        bg:C.greenBg,     color:C.green},
                {icon:"âš¡",label:"AI Analysis",        bg:C.primaryLight,color:C.primary,fn:()=>{setAiTarget(null);setShowAI(true);}},
                {icon:"ðŸ”„",label:"Refresh All",        bg:C.bg,          color:C.textSub,fn:loadAll},
              ].map(a=>(
          ...
          <div style={{textAlign:"center",padding:"20px 0 0",fontSize:11,color:C.textMute}}>
            Â© 2024 eLit. All rights reserved. &nbsp;Â·&nbsp; Dashboard v4.0 &nbsp;Â·&nbsp; AWS CloudWatch + EC2 Metrics Live
          </div>
          </div>}{/* end overview */}
        </div>
      </div>
