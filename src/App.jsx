...
async function fetchPerformance() {
  try {
    const res  = await fetch("http://localhost:3001/performance");
    const data = await res.json();
    return data;
  } catch { return null; }
}
...
async function fetchCost() {
  try {
    const res  = await fetch("http://localhost:3001/cost");
    const data = await res.json();
    return data;
  } catch { return null; }
}
...
async function fetchCanary() {
  try {
    const res  = await fetch("http://localhost:3001/canary");
    const data = await res.json();
    return data;
  } catch { return null; }
}
...
async function fetchRdsPerf() {
  try {
    const res  = await fetch("http://localhost:3001/rds-performance");
    const data = await res.json();
    return data;
  } catch { return null; }
}
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0,overflow:"auto"}}>
        ...
        {!loadingAlarms&&alarms.length===0&&(
          <div style={{background:C.amberBg,borderBottom:`1px solid ${C.amberBdr}`,padding:"10px 24px",fontSize:12,color:C.amber,fontWeight:600}}>
            ⚠ No alarms found. Make sure the backend is running at http://localhost:3001
          </div>
        )}
